...
UPLOAD_URL = "f6517f9c-d6f3-4c95-860a-283b5c7b767b"  
...
def upload_videos():
    changed_folder = os.path.join(folder_path.get(), "changed")
    upload_folder = os.path.join(folder_path.get(), "upload")
    os.makedirs(upload_folder, exist_ok=True)
    
    videos = [f for f in os.listdir(changed_folder) if f.lower().endswith(('.mp4', '.avi', '.mov', '.mkv'))]
    ...
    for video in videos:
        ...
        try:
            ...
            if response.status_code == 200:
                print(f"Upload successful: {video}")
                shutil.move(video_path, os.path.join(upload_folder, video))  # "sent" klasörüne taşı
            else:
                print(f"Upload failed: {video} - {response.status_code} {response.text}")
...
if __name__ == "__main__":
    ...
    root = tk.Tk()
    root.title("Video QR Scanner & Uploader")
    root.geometry("500x450")

    labelTitle = tk.Label(root, text="Video QR Scanner & Uploader", font=("Arial", 16, "bold"))
    labelTitle.pack(pady=10)

    folder_path = tk.StringVar()
    btnSelect = tk.Button(root, text="Folder Select", command=lambda: folder_path.set(filedialog.askdirectory()))
    btnSelect.pack()
    ...
    sys.stdout = textMultiOut  # Çıktıyı yönlendir

    btnStart = tk.Button(root, text="Start", command=start_process, bg="green", fg="white")
    btnStart.pack(pady=10)

    btnSend = tk.Button(root, text="Upload", command=start_upload, bg="blue", fg="white")

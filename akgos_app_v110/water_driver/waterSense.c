...
{
    WATER_STATE_INSTALLED_LF, WATER_STATE_INSTALLED_HF,
} waterState_t;
...
const int16_t thresholdTable[TOTAL_PADS] = { 440, // PAD 0 Threshold  // 475 // 519 // 779
        330, // PAD 1 Threshold  // 273 // 391 // 586
        330, // PAD 2 Threshold  // 385 // 548 // 823
        330, // PAD 3 Threshold  // 353 // 411 // 617
        440, // PAD 4 Threshold  // 427 // 475 // 712
        440  // PAD 5 Threshold  // 618 // 402 // 603
        };
/*
 ...
 */
const uint16_t highMarkFlowRates[7] = { 376, // Up through PAD 0 is covered with water
...
typedef struct csData_s
{
    uint8_t sample_data_number;
    uint16_t sample_pad_value[TOTAL_PADS];
    uint16_t past_sample_pad_value[TOTAL_PADS];
    uint8_t pass_sample_data[TOTAL_PADS];
} csData_t;
/*
 const uint16_t pad_max_Table[TOTAL_PADS] = { 45000, 45000, 45000, 45000, 45000,
 45000 };

 const uint16_t pad_min_Table[TOTAL_PADS] = { 32000, 30000, 32000, 32000, 32000,
 32000 };
 */
const uint16_t pad_max_Table = 45000;

const uint16_t pad_min_Table = 30000;
/**
 * \var wsData
...
uint16_t water_tester(char x){

  return wsData.padStats.padCounts[x];
}
/*************************
 * Module Prototypes
 ************************/
...
static uint32_t getSecondsSinceBoot(void);
...
void capsens_add_sample(void)
{
    ...
    csData.sample_data_number++;

    for (j = 0; j < TOTAL_PADS; j++)
    {
        uint16_t padCount = wsData.padStats.padCounts[j];
        if ((padCount > pad_min_Table) && (padCount < pad_max_Table))
        {
            csData.pass_sample_data[j]++;
            csData.sample_pad_value[j] += (pad_max_Table - padCount);
        }
    }

    if (csData.sample_data_number > 3)
    {

        for (j = 0; j < TOTAL_PADS; j++)
        {
            if (csData.pass_sample_data[j] == 0)
            {
                csData.sample_pad_value[j] = wsData.padStats.pad_max[j];
            }
            else
            {
                csData.sample_pad_value[j] = pad_max_Table
                        - csData.sample_pad_value[j]
                                / csData.pass_sample_data[j];
            }
        }

        waterSense_exec();
        memset(&csData, 0, sizeof(csData_t));
    }
...
void waterSense_init(void)
{
    ...
    wsData.senseDelayUntilNextMeas = 2;
...
void waterSense_exec(void)
{
  // if (!(P1IN & BIT0)) // BIT0 gsm enable  ?????????????? ****************************** problem ?????????????????????????????????????**************
   // {
        ...
        if ((wsData.lastTempMeasTime + TIME_60_SECONDS)
                <= getSecondsSinceBoot())
        {
       ...
       /* char str[100];
        ...
        sprintf(str, "%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n",
                timerSnGet(), readInternalTemperature(),
                wsData.padStats.padCounts[0] / 10,
                wsData.padStats.padCounts[1] / 10,
                wsData.padStats.padCounts[2] / 10,
                wsData.padStats.padCounts[3] / 10,
                wsData.padStats.padCounts[4] / 10,
                wsData.padStats.padCounts[5] / 10,
                wsData.padStats.padMeasDelta[0],
                wsData.padStats.padMeasDelta[1],
                wsData.padStats.padMeasDelta[2],
                wsData.padStats.padMeasDelta[3],
                wsData.padStats.padMeasDelta[4],
                wsData.padStats.padMeasDelta[5]);
        //  sprintf(str, "%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n", timerSnGet(),
         readInternalTemperature(), wsData.padStats.padMeasDelta[0],
         wsData.padStats.padMeasDelta[1], wsData.padStats.padMeasDelta[2],
         wsData.padStats.padMeasDelta[3], wsData.padStats.padMeasDelta[4],
         wsData.padStats.padMeasDelta[5]);
        sendString(str);*/
   // }else{
   //     wsData.senseDelayUntilNextMeas = 2;
...
static uint16_t waterSense_takeReading(void)
{
    ...
    for (j = 0; j < TOTAL_PADS; j++)
    {
        wsData.padStats.padCounts[j] = csData.sample_pad_value[j];
    }
    ...
    for (j = 0; j < TOTAL_PADS; j++)
    {
        ...
        if ((wsData.padStats.padMeasDelta[j] > 0)
                && (wsData.padStats.padMeasDelta[j] > thresholdVal))
        {
        ...
        /* // If this is a new max for the pad, record it
         ...
         if(wsData.pad_max_currentP[j] + 75 > wsData.padStats.padCounts[j]){
        ...
        if ((wsData.padStats.padCounts[j] > wsData.pad_max_nextP[j])
                && (wsData.prefillNextMaxTableFlag))
        {
...
static void doHighFreqReading(void)
{
    ...
    if (wsData.padStats.secondsOfNoWater > NO_WATER_HF_TO_LF_TIME_IN_SECONDS)
    {
        ...
        wsData.senseDelayUntilNextMeas = TIME_5_SECONDS;
    }
}
